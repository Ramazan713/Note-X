...
class CheckNoteArchiveKindState(private val context: CheckNoteEditorContext) :NoteStateAbstract<FragmentDisplayCheckListNoteBinding>(
    context
) {
    ...
    override fun setOnPrepareMenu(menu: Menu) {
        menu.findItem(R.id.achieve_remove_menu_item)?.isVisible=true
    ...
    override fun initExecutor(binding: FragmentDisplayCheckListNoteBinding) {
        noteKindListener?.invoke(NoteKinds.ARCHIVE_KIND)
    }
}